...
export function EventCard({ event, index, total, onOpen }: Props) {
  ...
  return (
    <button type="button" data-face onClick={() => onOpen(event)} aria-label={`View event: ${event.name}`}
      className="relative block size-full overflow-hidden border border-volt/40 bg-deep text-left will-change-transform">
      {showPoster ? (
        ...
        <span className="flex size-full flex-col justify-between p-4"
          ...
          <span className="font-display text-2xl font-bold leading-tight">{event.name}</span>
        </span>
      )}
    </button>
  );
