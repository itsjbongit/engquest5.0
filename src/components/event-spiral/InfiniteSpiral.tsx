...
type SpiralItem = {
  src?: string; alt?: string; label?: string; id?: string | number;
  href?: string; target?: string; code?: string;
};

type SpiralProps = {
  items?: (string | SpiralItem)[]; speed?: number; direction?: 'up' | 'down';
  animationMode?: 'auto' | 'drag' | 'scroll' | 'all'; radius?: number;
  cardWidth?: number; cardHeight?: number; verticalSpacing?: number;
  perspective?: number; cardsPerTurn?: number; rotation?: number; cardTilt?: number;
  cardRadius?: number; centerScale?: number; edgeFade?: number; edgeBlur?: number;
  pauseOnHover?: boolean; imageFit?: 'cover' | 'contain'; grayscale?: number; className?: string;
};
